...
const Motion = struct {
    ...
    fn set(self: *Motion, target: f32, now: u64, reduced: bool) void {
        if (reduced) {
            self.* = .{ .value = target, .target = target, .from = target };
    ...
    fn step(self: *Motion, now: u64) bool {
        ...
        const t = @min(1, @as(f32, @floatFromInt(now -| self.started)) / 180_000_000);
...
pub const State = struct {
    ...
    pub fn advance(self: *State, width: f32, now: u64, reduced: bool) bool {
        ...
        return dock_changed or overlay_changed or previous_dock != self.dock.value or previous_overlay != self.overlay.value or previous_compact != compact;
    }
